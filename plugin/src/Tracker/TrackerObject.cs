...
namespace PeakMapInteractive.Tracker
{
    ...
    internal static class TrackerObject
    {
        ...
        /// <summary>
        /// A collision shape that will not stand on an edge.
        ///
        /// The case's own box stood on its bottom edge when dropped upright,
        /// the way a book stands on a shelf. Here the back face is inset on
        /// every side, so every side face leans: upright, the lowest line is
        /// the front-bottom edge, the centre of mass sits behind it, and it
        /// falls over backwards — and cannot lean back onto the sloping
        /// bottom either, because that face is too short to catch it. The
        /// same on its head or on either side.
        ///
        /// It can still lie on its glass. Every attempt to shape that away —
        /// a ridge down the front, an off-centre one, a narrowed front — only
        /// produced a different flat pose to settle into, because a slab
        /// resting on a face has its weight inside that face whatever the
        /// face is. So a thrown device lands as a thrown thing lands; a
        /// device in a suitcase is placed on its back by
        /// <c>TrackerItem.LayInLuggage</c>, which is where it is found.
        ///
        /// None of this is visible: the inset is hidden inside the case, and
        /// nothing here changes how it is picked up or held.
        /// </summary>
        ...
        /// <summary>The live map, or null before there is one.</summary>
        internal static Texture Map => _map;

        /// <summary>What a screen looks like switched off.</summary>
        internal static readonly Color ScreenOff = new Color(0.035f, 0.055f, 0.065f, 1f);
        ...
        /// </summary>
        internal static void SetTexture(Material material, Texture texture)
        {
            bool set = false;
        ...
        internal static void SetColour(Material material, Color colour)
        {
            bool set = false;
